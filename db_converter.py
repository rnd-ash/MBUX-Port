...
class enum_object:
    def __init__(self, line: str, existing_names: []):
        ...
        self.key_name = self.key_name.encode("ascii", errors="ignore").decode() # Strip all the German characters from string (Compiler no likey!)
...
class code_object:
    ...
    #
    # TODO - Create subsystem in JVM for writing modified CAN Frames to bus
    #
    ...
    #        else:
    #            buf = "/** UNKNOWN DESCRIPTION **/\n"
    #        buf += "fun {0}(p: {1}) = ".format(self.__function_setter__, self.__data_type__)
    #        if self.__data_type__ == "Boolean": # Bo olean - just check return value is not 0
    #            buf += "setParam({0}, {1}, p.toInt())\n".format(self.__data_offset__, self.__data_length__)
    #        elif self.__data_type__ == "Int": # Int - return value is OK!
    #            buf += "setParam({0}, {1}, p)\n".format(self.__data_offset__, self.__data_length__)
    #        else: # Extract value from enum
    #            buf += "setParam({0}, {1}, {2}.raw)\n".format(self.__data_offset__, self.__data_length__, self.__data_type__)
    #        return buf
    #    except Exception as e:
    #        print("WARNING: Code generation error for {0} failed due to {1}".format(self.__name__, e))
    #        return ""
...
class frame_code:
    ...
    def generate_class_body(self) -> str: # Returns a all the code for the ECU Frame object
        ...
        for o in self.__code_objects__:
            ...
            #
            # TODO build method stubs in JVM for setters
            #
